...
function exportBackup(){
  ...
  const payload={schema:'civweave.chat.backup.v3',version:VERSION,exportedAt:now(),working:read(KEYS.working,{}),inbox:read(KEYS.inbox,[]),records:records(),validations:validations(),rewards:read(KEYS.rewards,{}),creator:creator(),peers:peers(),pairingInbox:pairingInbox(),settings:safeSettings,events:events(),projections:read(KEYS.projections,[])};
...
function restoreBackup(root){
  ...
  const b=parse(field(root,'backupJson'),null);if(!b||!/^civweave\.chat\.backup\.v[23]$/.test(b.schema||''))throw new Error('Unsupported Civweave Chat backup.');
  write(KEYS.working,b.working||{});write(KEYS.inbox,Array.isArray(b.inbox)?b.inbox:[]);write(KEYS.records,Array.isArray(b.records)?b.records:[]);write(KEYS.validations,Array.isArray(b.validations)?b.validations:[]);write(KEYS.rewards,b.rewards||{});write(KEYS.creator,b.creator||{});write(KEYS.peers,Array.isArray(b.peers)?b.peers:[]);write(KEYS.pairingInbox,Array.isArray(b.pairingInbox)?b.pairingInbox:[]);write(KEYS.settings,normalizedSettings({...b.settings,allowRemotePrompts:false}));write(KEYS.events,Array.isArray(b.events)?b.events:[]);write(KEYS.projections,Array.isArray(b.projections)?b.projections:[]);write(KEYS.paymentReceipts,[]);localStorage.removeItem(PAYMENT_SESSION_KEY);audit('backup.restored',{remotePromptsReset:true,paymentAssertionsDropped:true});return true
}
